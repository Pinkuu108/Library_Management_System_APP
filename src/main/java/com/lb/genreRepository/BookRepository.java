...
public interface BookRepository extends JpaRepository<Book, Long> {
	...
	@Query("select b from Book b where " + "(:searchTerm is null OR "
			+ "lower(b.title) like lower(concat('%', :searchTerm, '%')) OR "
			+ "lower(b.author) like lower(concat('%', :searchTerm, '%')) OR "
			+ "lower(b.isbn) like lower(concat('%', :searchTerm, '%'))) AND "
			+ "(:genreId is null or b.genre.id = :genreId) AND "
			+ "(:availableOnly = false OR b.availableCopies > 0) AND " + "b.active = true")
	Page<Book> searchBooksWithFilters(@Param("searchTerm") String searchTerm, @Param("genreId") Long genreId,
